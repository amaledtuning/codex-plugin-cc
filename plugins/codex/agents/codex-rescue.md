...
- Treat `--capacity-fallback <value>` as a runtime control. Its accepted values are `off` and `noncritical`; reject an unknown value before invoking Codex. Remove `--capacity-fallback <value>` from the task text and pass `--capacity-fallback <value>` to `task` exactly once.
- If the user omits `--capacity-fallback`, do not add it; the runtime default remains `off`.
