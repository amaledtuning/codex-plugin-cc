...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [--capacity-fallback <off|noncritical>] [what Codex should investigate, solve, or continue]"
...
- `--capacity-fallback` is a runtime-selection flag. Its accepted values are `off` and `noncritical`. If another value is supplied, reject the request before invoking the subagent.
- Preserve a valid capacity-fallback selection for the forwarding subagent. It must remove `--capacity-fallback <value>` from the natural-language task text and forward it to `task` exactly once. If omitted, do not add it; the runtime default remains `off`.
